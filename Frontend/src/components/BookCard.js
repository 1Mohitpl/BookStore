  ...
  return (
    <div className="bg-white rounded-2xl shadow-md overflow-hidden transition-all duration-300 hover:-translate-y-2 hover:shadow-xl hover:border-indigo-300 border border-slate-200 flex flex-col h-full group">
      ...
      <div className="p-6 flex-1 flex flex-col">
        <h3 className="text-xl font-semibold mb-2 text-slate-900 break-words leading-snug">
          {book.title}
        </h3>
        <p className="text-slate-500 text-sm mb-1 flex items-center gap-2">by {book.author}</p>
        <p className="inline-block text-pink-600 bg-pink-50 px-3 py-1 rounded-full text-xs my-2 font-semibold uppercase tracking-wide w-max">
          {book.category}
        </p>
        <p className="text-slate-500 text-sm my-3 flex-1 overflow-hidden line-clamp-3 leading-relaxed">
          {book.desc}
        </p>

        <div className="flex justify-between items-center my-4 pt-4 border-t border-slate-200">
          <span className="bg-slate-100 text-slate-500 px-3 py-1 rounded-md text-sm font-medium">
            {book.language}
          </span>
