...
const Admin = () => {
    ...
    if (!isAdmin) {
        return <div className="max-w-4xl mx-auto px-6 py-16 text-center text-xl text-slate-500"><p>Access denied. Admin only.</p></div>;
    }
    ...
    return (
        <div className="max-w-7xl mx-auto px-6 py-12">
            ...
            {showForm && (
                <form onSubmit={handleSubmit} className="bg-white rounded-2xl p-8 shadow-sm border border-slate-200 mb-10 max-w-3xl">
                    ...
                    <div className="flex flex-col sm:flex-row gap-4">
                        <button type="submit" className="flex-1 bg-gradient-to-r from-indigo-600 to-violet-600 text-white font-bold py-3.5 rounded-xl shadow-md hover:-translate-y-0.5 transition-all">
                            {editingId ? 'Update Book' : 'Add Book'}
                        </button>
                        <button type="button" onClick={resetForm} className="flex-1 bg-slate-100 text-slate-700 border border-slate-200 font-bold py-3.5 rounded-xl hover:bg-slate-200 transition-all">
                            Cancel
                        </button>
                    </div>
                </form>
            ...
            <div className="bg-white rounded-2xl p-8 shadow-sm border border-slate-200">
                <h2 className="text-2xl font-bold text-slate-900 mb-8 border-b border-slate-200 pb-4">Inventory ({books.length})</h2>
                {loading ? (
                    <p className="text-center py-10 text-slate-500 font-medium animate-pulse">Loading books...</p>
                ) : books.length === 0 ? (
