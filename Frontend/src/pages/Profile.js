...
const Profile = () => {
    ...
    return (
        <div className="max-w-3xl mx-auto px-6 py-12">
            ...
            {message && <div className="bg-emerald-50 text-emerald-600 p-4 rounded-lg mb-8 border-l-4 border-emerald-500 font-medium">{message}</div>}
                ...
                {!isEditing ? (
                    <div className="flex flex-col gap-6">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-[150px_1fr] gap-2 md:gap-4 items-center">
                            ...
                            <span className="text-lg text-slate-900 break-words">{user.address || 'Not provided'}</span>
                        </div>

                        <button onClick={() => setIsEditing(true)} className="mt-4 bg-gradient-to-r from-indigo-600 to-violet-600 text-white px-8 py-3 rounded-lg font-semibold shadow-md hover:-translate-y-0.5 hover:shadow-lg transition-all self-start">
                            Edit Profile
                        </button>
                    </div>
                ) : (
                    <form onSubmit={handleSubmit} className="flex flex-col gap-6">
                        ...
                        <div className="flex flex-col sm:flex-row gap-4 mt-4">
                            <button type="submit" disabled={loading} className="flex-1 bg-gradient-to-r from-emerald-500 to-emerald-600 text-white font-bold py-3.5 rounded-xl shadow-md hover:-translate-y-0.5 transition-all disabled:opacity-50">
            ...
            <div className="bg-white rounded-2xl p-8 md:p-10 shadow-sm border border-slate-200">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <button onClick={() => navigate('/orders')} className="bg-slate-50 text-slate-700 border border-slate-200 p-4 rounded-xl font-semibold hover:border-indigo-500 hover:text-indigo-600 hover:-translate-y-1 transition-all">
                        View My Orders
                    </button>
                    <button onClick={() => navigate('/books')} className="bg-slate-50 text-slate-700 border border-slate-200 p-4 rounded-xl font-semibold hover:border-indigo-500 hover:text-indigo-600 hover:-translate-y-1 transition-all">
                        Continue Shopping
                    </button>
                </div>
            </div>
