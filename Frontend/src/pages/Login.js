...
const Login = () => {
    ...
    return (
        <div className="min-h-screen flex items-center justify-center p-6 bg-slate-50 bg-[radial-gradient(ellipse_at_top,_var(--tw-gradient-stops))] from-indigo-50 via-slate-50 to-pink-50">
            <div className="w-full max-w-md bg-white/80 backdrop-blur-xl p-8 md:p-10 rounded-3xl shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-white/50 relative overflow-hidden">
                ...
                <div className="relative z-10">
                    <h2 className="text-3xl font-bold text-center mb-8 text-slate-800">Login to <span className="text-transparent bg-clip-text bg-gradient-to-r from-indigo-600 to-violet-600">BookStore</span></h2>
                    ...
                    <form onSubmit={handleSubmit} className="flex flex-col gap-6">
                        ...
                        <button type="submit" disabled={loading} className="w-full bg-gradient-to-r from-indigo-600 to-violet-600 text-white font-bold py-4 rounded-2xl shadow-lg shadow-indigo-200 hover:shadow-xl hover:-translate-y-1 hover:from-indigo-500 hover:to-violet-500 transition-all duration-300 mt-2 disabled:opacity-70 disabled:cursor-not-allowed disabled:-translate-y-0 disabled:shadow-none">
                            {loading ? 'Logging in...' : 'Login'}
                        </button>
                    </form>

                    <p className="text-center mt-8 text-sm font-medium text-slate-500">
                        Don't have an account? <Link to="/signup" className="text-indigo-600 hover:text-indigo-800 hover:underline">Sign up here</Link>
                    </p>
                </div>
            </div>
        </div>
